<template>
  ...
  <!-- Previous Item -->
  <div class="navigation-area previous" v-if="index && index > 0" @click="previous()">
...
<script lang="ts">
import Cast from '@/components/CastComponent.vue';
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      nextListener: (e: KeyboardEvent) => {
      if (e.key == 'ArrowRight') {
        this.next();
      }
    },
      previousListener: (e: KeyboardEvent) => {
      if (e.key == 'ArrowLeft') {
        this.previous();
      }
    },
      closeListener: (e: KeyboardEvent) => {
      if (e.key == 'Escape') {
        this.close();
      }
    },
  ...
  async created() {
    ...
    document.addEventListener('keydown', this.nextListener);

    // Previous item on arrow left
    document.addEventListener('keydown', this.previousListener);
  ...
  methods: {
    ...
    async loadItem() {
      ...
      if (!this.item || !dimensions) return;
     ...
     */
    getScreenDimensions() {
      const elem = this.$refs.item as Element;
